...
func StringToTime(str_time string) (time.Time) {
    if str_time == "" {
        return time.Time{}
    }

    tlayout := "2006-01-02 15:04:05.000000"

    dt, err := time.Parse(tlayout, str_time)
    if err != nil {
        ERROR("core.converter.StringToTime:" + err.Error())
        return dt
    }
    return dt
}
...
func StringToTimeWithoutMicrosec(str_time string) (time.Time, error) {
    if str_time == "" {
...
func ToFloat64Array(i1 interface{}) []float64 {
    if i1 == nil {
        return []float64{}
    }
    switch i2 := i1.(type) {
...
func ToArrayFloat64Array(i1 interface{}) [][]float64 {
    if i1 == nil {
        return [][]float64{}
    }
    switch i2 := i1.(type) {
...
func ToTime(i1 interface{}) (time.Time) {
    if i1 == nil {
        return time.Time{}
    }
    switch i2 := i1.(type) {
    default:
        ERROR(fmt.Sprintf("Error in ToTime!!! i2 type: %v", reflect.TypeOf(i2)))
        return time.Time{}
    case uint64:
        return time.Unix(int64(i2), 0)
    case []uint8:
        str := string(i2)
